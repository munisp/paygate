...
class WealthManagementScreen extends ConsumerStatefulWidget {
  const WealthManagementScreen({super.key});
  @override
  ConsumerState<WealthManagementScreen> createState() => _WealthManagementScreenState();
}

class _WealthManagementScreenState extends ConsumerState<WealthManagementScreen> {
    ...
    try {
      ...
      setState(() {
        _items = List<Map<String, dynamic>>.from(
          (data['assets'] ?? data['data'] ?? data['result'] ?? []) as List,
        );
        _loading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(
        title: const Text('Wealth Management', style: TextStyle(color: _text)),
        backgroundColor: _bg,
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  (a['name'] ?? a['assetType'] ?? 'Asset').toString(),
                                  style: const TextStyle(color: _text, fontWeight: FontWeight.w600),
                                ),
                                subtitle: Text(
                                  (a['value']?.toString() ?? '0').toString(),
