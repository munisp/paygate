...
class FeeSchedulesScreen extends ConsumerStatefulWidget {
  const FeeSchedulesScreen({super.key});
  @override
  ConsumerState<FeeSchedulesScreen> createState() => _FeeSchedulesScreenState();
}

class _FeeSchedulesScreenState extends ConsumerState<FeeSchedulesScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadData({bool refresh = false}) async {
    ...
    try {
      ...
      final result = await api.listFeeSchedules(page: _page);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Fee Schedules'),
        actions: [
          IconButton(
        ...
        child: _error != null
            ? Center(
                child: Column(
                  ...
                  children: [
                    Icon(Icons.price_change, size: 48, color: Colors.red[300]),
                ...
                : _items.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.price_change, size: 64, color: Colors.teal[200]),
                            const SizedBox(height: 16),
                            Text('No Fee Schedules found', style: Theme.of(context).textTheme.titleMedium),
                            const SizedBox(height: 8),
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                leading: CircleAvatar(
                                  ...
                                  child: Icon(Icons.price_change, color: Colors.teal[700], size: 20),
                                ),
                                title: Text(
                                  item['name'] ?? item['label'] ?? 'Schedule',
