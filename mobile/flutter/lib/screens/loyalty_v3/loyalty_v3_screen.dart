...
class LoyaltyV3Screen extends ConsumerStatefulWidget {
  const LoyaltyV3Screen({super.key});
  @override
  ConsumerState<LoyaltyV3Screen> createState() => _LoyaltyV3ScreenState();
}

class _LoyaltyV3ScreenState extends ConsumerState<LoyaltyV3Screen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadData({bool refresh = false}) async {
    ...
    try {
      ...
      final result = await api.listLoyaltyV3Campaigns(page: _page);
        ...
        child: _error != null
            ? Center(
                child: Column(
                  ...
                  children: [
                    Icon(Icons.stars, size: 48, color: Colors.red[300]),
                ...
                : _items.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.stars, size: 64, color: Colors.amber[200]),
                            const SizedBox(height: 16),
                            Text('No Loyalty Programs found', style: Theme.of(context).textTheme.titleMedium),
                            const SizedBox(height: 8),
