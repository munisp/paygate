...
class ClaimDocumentsScreen extends ConsumerStatefulWidget {
  const ClaimDocumentsScreen({super.key});
  @override
  ConsumerState<ClaimDocumentsScreen> createState() => _ClaimDocumentsScreenState();
}

class _ClaimDocumentsScreenState extends ConsumerState<ClaimDocumentsScreen> {
    ...
    try {
      ...
      setState(() {
        _items = List<Map<String, dynamic>>.from(
          (data['documents'] ?? data['data'] ?? data['result'] ?? []) as List,
        );
        _loading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(
        title: const Text('Claim Documents', style: TextStyle(color: _text)),
        backgroundColor: _bg,
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  (doc['documentType'] ?? doc['type'] ?? 'Document').toString(),
