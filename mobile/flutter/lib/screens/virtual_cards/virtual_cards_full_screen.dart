...
class VirtualCardsFullScreen extends ConsumerStatefulWidget {
  const VirtualCardsFullScreen({super.key});
  @override
  ConsumerState<VirtualCardsFullScreen> createState() => _VirtualCardsFullScreenState();
}

class _VirtualCardsFullScreenState extends ConsumerState<VirtualCardsFullScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadData({bool refresh = false}) async {
    ...
    try {
      ...
      final result = await api.listVirtualCards(page: _page);
        ...
        child: _error != null
            ? Center(
                child: Column(
                  ...
                  children: [
                    Icon(Icons.credit_card, size: 48, color: Colors.red[300]),
                ...
                : _items.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.credit_card, size: 64, color: Colors.blueGrey[200]),
                            const SizedBox(height: 16),
                            Text('No Virtual Cards found', style: Theme.of(context).textTheme.titleMedium),
                            const SizedBox(height: 8),
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                leading: CircleAvatar(
                                  ...
                                  child: Icon(Icons.credit_card, color: Colors.blueGrey[700], size: 20),
                                ),
                                title: Text(
                                  item['maskedPan'] ?? item['label'] ?? 'Card',
                                  style: const TextStyle(fontWeight: FontWeight.w600),
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                ),
                                subtitle: Text(
                                  item['holderName'] ?? item['currency'] ?? '',
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                ),
                                trailing: Container(
