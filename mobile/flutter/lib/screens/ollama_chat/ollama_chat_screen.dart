...
class OllamaChatScreen extends ConsumerStatefulWidget {
  const OllamaChatScreen({super.key});
  @override
  ConsumerState<OllamaChatScreen> createState() => _OllamaChatScreenState();
}

class _OllamaChatScreenState extends ConsumerState<OllamaChatScreen> {
    ...
    try {
      ...
      setState(() {
        _items = List<Map<String, dynamic>>.from(
          (data['messages'] ?? data['data'] ?? data['result'] ?? []) as List,
        );
        _loading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(
        title: const Text('AI Chat', style: TextStyle(color: _text)),
        backgroundColor: _bg,
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  (msg['role'] ?? 'user').toString(),
                                  style: const TextStyle(color: _text, fontWeight: FontWeight.w600),
                                ),
                                subtitle: Text(
                                  (msg['content']?.toString().substring(0, 50) ?? '').toString(),
