...
class ComplianceKYCScreen extends ConsumerStatefulWidget {
  const ComplianceKYCScreen({super.key});
  @override
  ConsumerState<ComplianceKYCScreen> createState() => _ComplianceKYCScreenState();
}

class _ComplianceKYCScreenState extends ConsumerState<ComplianceKYCScreen> {
    ...
    try {
      ...
      setState(() {
        _items = List<Map<String, dynamic>>.from(
          (data['applications'] ?? data['data'] ?? data['result'] ?? []) as List,
        );
        _loading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(
        title: const Text('Compliance KYC', style: TextStyle(color: _text)),
        backgroundColor: _bg,
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return Container(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  (app['merchantId'] ?? app['id']).toString(),
