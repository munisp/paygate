...
class _ReportsCenterScreenState extends ConsumerState<ReportsCenterScreen> {
    ...
    try {
      ...
      setState(() {
        _items = List<Map<String, dynamic>>.from(
          (data['reports'] ?? data['data'] ?? data['result'] ?? []) as List,
        );
        _loading = false;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _bg,
      appBar: AppBar(
        title: const Text('Reports Center', style: TextStyle(color: _text)),
        backgroundColor: _bg,
