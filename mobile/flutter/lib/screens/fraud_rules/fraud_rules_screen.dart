...
class FraudRulesScreen extends ConsumerStatefulWidget {
  const FraudRulesScreen({super.key});
  @override
  ConsumerState<FraudRulesScreen> createState() => _FraudRulesScreenState();
}

class _FraudRulesScreenState extends ConsumerState<FraudRulesScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _loadData({bool refresh = false}) async {
    ...
    try {
      ...
      final result = await api.listFraudRules(page: _page);
        ...
        child: _error != null
            ? Center(
                child: Column(
                  ...
                  children: [
                    Icon(Icons.security, size: 48, color: Colors.red[300]),
                ...
                : _items.isEmpty
                    ? Center(
                        child: Column(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Icon(Icons.security, size: 64, color: Colors.red[200]),
                            const SizedBox(height: 16),
                            Text('No Fraud Rules found', style: Theme.of(context).textTheme.titleMedium),
                            const SizedBox(height: 8),
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            return Card(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  item['name'] ?? item['label'] ?? 'Rule',
                                  style: const TextStyle(fontWeight: FontWeight.w600),
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                ),
                                subtitle: Text(
                                  item['description'] ?? item['ruleType'] ?? '',
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                ),
                                trailing: Container(
