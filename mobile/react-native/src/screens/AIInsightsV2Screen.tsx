...
const AIInsightsV2Screen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.ai.insights
      const response = await fetch('/api/trpc/ai.insights?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading AI Insights V2...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>AI Insights V2</Text>
      <FlatList
        data={data}
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No AI Insights V2 data available.</Text>
        }
      />
    </View>
  );
...
export default AIInsightsV2Screen;
