...
const CorridorLiveStatsScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.corridorLive.list
      const response = await fetch('/api/trpc/corridorLive.list?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Corridor Live Stats...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Corridor Live Stats</Text>
      <FlatList
        data={data}
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No Corridor Live Stats data available.</Text>
        }
      />
    </View>
  );
...
export default CorridorLiveStatsScreen;
