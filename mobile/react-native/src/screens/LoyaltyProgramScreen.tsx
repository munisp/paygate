 ...
 * Wired to tRPC procedure: loyalty.getProgram
 */
export default function LoyaltyProgramScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['loyalty']?.['getProgram']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.loadingText}>Loading Loyalty...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load Loyalty</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        accessibilityLabel="Loyalty content"
      >
        {data ? (
