...
/**
 * SIP Investments Screen
 * Auto-generated parity screen for PayGate Merchant Portal mobile app.
 * Wired to tRPC procedure: sip.listPlans
 */
export default function SIPInvestmentsScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['sip']?.['listPlans']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load SIP Investments</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>SIP Investments</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <ScrollView
        ...
        accessibilityLabel="SIP Investments content"
      >
        {data ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>No SIP Investments Data</Text>
