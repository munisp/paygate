...
const TenantApiKeysScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.apiKeys.list
      const response = await fetch('/api/trpc/apiKeys.list?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Tenant API Keys...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Tenant API Keys</Text>
      <FlatList
        data={data}
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No Tenant API Keys data available.</Text>
        }
      />
    </View>
  );
...
export default TenantApiKeysScreen;
