...
const SubscriptionBillingV2Screen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.stripeSubscriptions.list
      const response = await fetch('/api/trpc/stripeSubscriptions.list?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Subscription Billing V2...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Subscription Billing V2</Text>
      <FlatList
        data={data}
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No Subscription Billing V2 data available.</Text>
        }
      />
    </View>
  );
...
export default SubscriptionBillingV2Screen;
