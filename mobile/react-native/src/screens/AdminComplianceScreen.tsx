 ...
 * Wired to tRPC procedure: admin.getComplianceReports
 */
export default function AdminComplianceScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['admin']?.['getComplianceReports']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load Compliance</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        accessibilityLabel="Compliance content"
      >
        {data ? (
