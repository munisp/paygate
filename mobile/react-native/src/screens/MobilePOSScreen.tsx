...
const MobilePOSScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.pos.transactions
      const response = await fetch('/api/trpc/pos.transactions?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Mobile POS...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Mobile POS</Text>
      <FlatList
        data={data}
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No Mobile POS data available.</Text>
        }
      />
    </View>
  );
