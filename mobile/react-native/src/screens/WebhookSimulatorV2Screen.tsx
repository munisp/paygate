...
const WebhookSimulatorV2Screen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.webhooks.list
      const response = await fetch('/api/trpc/webhooks.list?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Webhook Simulator V2...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Webhook Simulator V2</Text>
      <FlatList
        data={data}
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No Webhook Simulator V2 data available.</Text>
        }
      />
    </View>
  );
...
export default WebhookSimulatorV2Screen;
