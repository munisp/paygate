 ...
 * Wired to tRPC procedure: admin.getPlatformStats
 */
export default function AdminPlatformOverviewScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['admin']?.['getPlatformStats']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load Platform Overview</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        accessibilityLabel="Platform Overview content"
      >
        {data ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>No Platform Overview Data</Text>
