 ...
 * Wired to tRPC procedure: escrow.list
 */
export default function EscrowAccountsScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['escrow']?.['list']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load Escrow</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        accessibilityLabel="Escrow content"
      >
        {data ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>No Escrow Data</Text>
