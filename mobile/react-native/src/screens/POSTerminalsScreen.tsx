 ...
 * Wired to tRPC procedure: pos.listTerminals
 */
export default function POSTerminalsScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['pos']?.['listTerminals']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load POS Terminals</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>POS Terminals</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <ScrollView
        ...
        accessibilityLabel="POS Terminals content"
      >
        {data ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>No POS Terminals Data</Text>
