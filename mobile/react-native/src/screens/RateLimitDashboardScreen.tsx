...
const RateLimitDashboardScreen: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      setError(null);
      // TODO: wire to real tRPC endpoint: trpc.waf.alerts
      const response = await fetch('/api/trpc/waf.alerts?input={}', {
        credentials: 'include',
      });
  ...
  if (isLoading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.loadingText}>Loading Rate Limit Dashboard...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.emptyText}>No Rate Limit Dashboard data available.</Text>
        }
      />
    </View>
  );
