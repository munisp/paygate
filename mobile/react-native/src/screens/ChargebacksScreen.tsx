 ...
 * Wired to tRPC procedure: chargebacks.list
 */
export default function ChargebacksScreen() {
  const navigation = useNavigation();
  const [refreshing, setRefreshing] = useState(false);

  const { data, isLoading, error, refetch } = (trpc as any)['chargebacks']?.['list']?.useQuery?.() ?? { data: null, isLoading: false, error: null, refetch: async () => {} };
  ...
  if (error) {
    return (
      <SafeAreaView style={styles.container}>
        <StatusBar barStyle="light-content" backgroundColor={colors.background} />
        <View style={styles.centered}>
          <Text style={styles.errorText}>Failed to load Chargebacks</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        accessibilityLabel="Chargebacks content"
      >
        {data ? (
          ...
          <View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>No Chargebacks Data</Text>
