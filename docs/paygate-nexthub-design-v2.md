...
## Part XVI — NextHub-PayGate Integration Architecture (Wave 218)

### XVI.1 Architectural Position

PayGate's existing feature set (Transactions, Payouts, Settlements, Fraud & Risk, Compliance, Billing, Subscriptions, Virtual Cards, FX, BNPL, Loyalty) operates on a **tRPC + Express + Drizzle ORM + PostgreSQL** stack. NextHub sits as an **interoperability sidecar** that extends this stack without replacing it.

```
┌─────────────────────────────────────────────────────────────────┐
│                     Merchant Portal (React)                      │
│  Existing Pages          │   NextHub Domain Pages (W210–W217)   │
└──────────────────────────┼──────────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────────┐
│              tRPC API Layer (Express + routers.ts)               │
│  Existing routers        │   nexthub* + wave211_217 + wave218   │
└──────────────────────────┼──────────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────────┐
│                   Apache APISIX (API Gateway)                    │
│  /api/trpc/*             │   /nexthub/* (FSPIOP + domain APIs)  │
└──────────────────────────┼──────────────────────────────────────┘
                           │
       ┌───────────────────┼─────────────────────┐
       │                   │                     │
┌──────▼──────┐   ┌────────▼────────┐   ┌───────▼──────┐
│  Go FSPIOP  │   │  Temporal Worker │   │  Python ML   │
│  Handlers   │   │  (Workflows)     │   │  Services    │
└──────┬──────┘   └────────┬────────┘   └───────┬──────┘
       │                   │                     │
┌──────▼───────────────────▼─────────────────────▼──────┐
│           TigerBeetle (Ledger) + PostgreSQL (State)    │
└────────────────────────────────────────────────────────┘
```

### XVI.2 Integration Points

The following table maps each existing PayGate feature to its NextHub complement or replacement:

| PayGate Feature | NextHub Integration | Mode |
|---|---|---|
| Transactions (`/transactions`) | All domain transactions feed into the unified `crossDomainSearch` router | **Complement** — domain transactions are separate tables but searchable together |
| FX & Rates (`/fx`) | `nexthubFXRouter` replaces the static FX rate store with live corridor management and FSPIOP FX API v2.0 | **Replace** (for cross-border flows) |
| Settlements (`/settlements`) | `nexthubSettlementRouter` adds DNS/RTGS mode, multilateral netting, and TigerBeetle atomic settlement | **Complement** — existing settlement for card/NIP; NextHub for FSPIOP/RTGS |
| Reconciliation (`/reconciliation`) | `nexthubReconciliationRouter` adds Temporal-driven 5-step recon workflow with auto-posting | **Complement** — existing recon for card; NextHub for interbank/FSPIOP |
| Fraud & Risk (`/fraud-risk`) | `complianceFlagRouter` (W218) propagates AML/sanctions flags across all 7 domain tables | **Complement** — existing rule engine + NextHub cross-domain propagation |
| Compliance & KYC (`/compliance`) | `nexthubSecurityRouter` adds mTLS, OpenBao secrets, Permify RBAC, and SIEM event forwarding | **Complement** — existing KYC for onboarding; NextHub for runtime access control |
| Billing Engine (`/billing-engine`) | `nexthubBillingRouter` adds per-domain fee posting to TigerBeetle and invoice generation | **Complement** — existing billing for subscriptions; NextHub for domain transaction fees |
| Disputes (`/disputes`) | `nexthubDisputesRouter` adds FSPIOP error code mapping and Temporal dispute resolution workflow | **Complement** — existing disputes for card chargebacks; NextHub for FSPIOP disputes |
| API Keys (`/api-keys`) | `domainApiKeyRouter` (W218) adds per-domain API keys with APISIX rate limiting | **Complement** — existing keys for portal access; domain keys for external API consumers |
| Webhooks (`/webhooks`) | Domain events (claim.submitted, transfer.completed, vend.token.generated) are dispatched via existing `webhookDispatch` | **Complement** — same delivery infrastructure, new event types |
| Analytics (`/analytics`) | `domainAnalyticsRouter` (W218) adds hourly TPS buckets and cross-domain volume summary | **Complement** — existing analytics for card/NIP; domain analytics for W211–W217 |

### XVI.3 Data Flow: PayGate Transaction → NextHub Settlement

When a merchant initiates a cross-border payment through the existing `/transactions` endpoint, the flow is:

1. **PayGate tRPC** validates the transaction and records it in PostgreSQL.
2. **Middleware Bridge** (`middlewareBridge.ts`) calls `recordTransactionViaMiddleware()` which forwards to the Go FSPIOP handler.
3. **Go FSPIOP Handler** translates the PayGate transaction to a Mojaloop/FSPIOP transfer request, performs ALS lookup, and calls the payee DFSP.
4. **Temporal Workflow** (`TransferWorkflow`) orchestrates the 5-step FSPIOP saga (quote → prepare → fulfil → notify → settle).
5. **TigerBeetle** records the double-entry debit/credit atomically.
6. **NextHub Settlement Router** updates the settlement position and triggers DNS/RTGS netting if configured.
7. **PayGate Billing Engine** posts the platform fee via `nexthubBillingRouter`.
8. **Webhook Dispatch** fires `transfer.completed` to the merchant's registered webhook endpoint.

### XVI.4 Mojaloop Drop-In Replacement

NextHub is a **drop-in replacement** for Mojaloop in the following sense: any DFSP or third-party fintech that speaks FSPIOP v1.1 or v2.0 can connect to NextHub's APISIX gateway without modification. The Go handlers implement the same REST contract as Mojaloop's ml-api-adapter. The key differences are:

| Dimension | Mojaloop | NextHub |
|---|---|---|
| Ledger | Custom settlement model | TigerBeetle (128-bit IDs, atomic double-entry) |
| Workflow engine | Custom state machines | Temporal (durable, retryable, observable) |
| Billing | Not included | Built-in fee engine with TigerBeetle posting |
| AML/Compliance | Not included | Rules engine + STR filing + NFIU integration |
| Domain expansion | Payments only | 7 additional domains (W211–W217) |
| Observability | Basic logging | OpenTelemetry + Prometheus + Grafana |
| Multi-tenancy | Single hub | Per-merchant RBAC via Permify |
| CBDC support | Not included | eNaira, ECB TIPS, FedNow, DCEP (W217) |

---

## Part XVII — APISIX API Key Management and External Connectivity

### XVII.1 Architecture

Apache APISIX serves as the single ingress point for all external API consumers (third-party fintechs, government systems, banking products). It enforces authentication, rate limiting, request transformation, and observability before any request reaches the NextHub Go handlers or the PayGate tRPC layer.

```
External Consumer (Fintech / Govt / Bank)
         │
...
│  1. key-auth       → validate API key SHA-256 hash  │
│  2. limit-count    → rate limit (RPM, sliding window)│
│  3. jwt-auth       → optional JWT for PISP flows    │
│  4. request-id     → inject X-Request-ID header     │
│  5. opentelemetry  → emit span to OTEL collector    │
│  6. response-rewrite → add CORS + security headers  │
│                                                      │
│  Routes:                                             │
│  /nexthub/remittance/*  → nexthub-go:8080           │
│  /nexthub/healthcare/*  → nexthub-go:8080           │
│  /nexthub/insurance/*   → nexthub-go:8080           │
│  /nexthub/scf/*         → nexthub-go:8080           │
│  /nexthub/g2p/*         → nexthub-go:8080           │
│  /nexthub/energy/*      → nexthub-go:8080           │
│  /nexthub/cbdc/*        → nexthub-go:8080           │
│  /nexthub/travel-rule/* → travel-rule-py:8000       │
│  /nexthub/nhia/*        → nhia-py:8001              │
│  /api/trpc/*            → portal-node:3000          │
└─────────────────────────────────────────────────────┘
```

### XVII.2 API Key Lifecycle

API keys are managed through the `domainApiKeyRouter` (Wave 218) in the merchant portal. The lifecycle is:

**Creation.** A merchant creates a domain-scoped API key via the portal (`POST /api/trpc/wave218.domainApiKeys.create`). The server generates a 48-byte random key, stores a SHA-256 hash in PostgreSQL (`domain_api_keys` table), and returns the raw key once. The key prefix (first 20 characters) is stored for display purposes.

**APISIX Sync.** On creation, the portal calls the APISIX Admin API to register the key hash as a consumer with the `key-auth` plugin. The rate limit (RPM) is set via the `limit-count` plugin on the consumer's route group.

**Validation.** When an external consumer sends `X-API-Key: pg_remittance_<48-hex>`, APISIX computes SHA-256 of the key and matches it against the consumer registry. If matched, the request is forwarded with the consumer ID injected as `X-Consumer-ID`.

**Rotation.** The merchant revokes the old key (sets `is_active = false` in PostgreSQL and deletes the APISIX consumer) and creates a new one. The old key is immediately rejected.

**Audit.** Every API key usage is logged to `domain_audit_log` via the APISIX `opentelemetry` plugin, which forwards spans to the OTEL collector.

### XVII.3 Rate Limiting Strategy

Rate limits are enforced at three levels:

| Level | Mechanism | Default |
|---|---|---|
| Per API key | APISIX `limit-count` plugin (sliding window) | 60 RPM |
| Per domain per IP | APISIX `limit-req` plugin (token bucket) | 100 RPS |
| Platform-wide | APISIX `limit-conn` plugin (concurrent connections) | 10,000 |

Domain-specific defaults reflect the expected transaction volumes:

| Domain | Default RPM | Burst Allowance |
|---|---|---|
| Remittance | 60 | 2× for 10s |
| Healthcare | 120 | 3× for 30s (batch claim submission) |
| Insurance | 60 | 2× for 10s |
| SCF | 30 | 2× for 10s |
| G2P | 300 | 5× for 60s (bulk disbursement) |
| Energy | 600 | 10× for 30s (meter vending peak) |
| CBDC | 1,200 | 20× for 10s (retail CBDC) |

### XVII.4 How Third-Party Fintechs, Government Systems, and Banks Connect

**Step 1 — Onboarding.** The third party registers as a merchant in the PayGate portal, completes KYB verification, and selects the domain(s) they wish to access.

**Step 2 — API Key Generation.** Via the portal's Domain Overview page (`/domains/overview`), the merchant generates a domain-scoped key. The key is shown once and must be stored securely.

**Step 3 — DFSP Registration (for FSPIOP flows).** For remittance or CBDC flows, the third party is registered as a DFSP in the NextHub ALS via `nexthubDfspsRouter`. This assigns them a `dfspId` and registers their callback URL.

**Step 4 — Integration.** The third party calls NextHub APIs using the API key:

```http
POST /nexthub/remittance/transfers
X-API-Key: pg_remittance_<48-hex>
Content-Type: application/json

{
  "corridorId": "NGN-GBP-001",
  "senderName": "Adebayo Okafor",
...
**Step 5 — Webhook Registration.** The third party registers a webhook endpoint via `POST /api/trpc/webhooks.create` to receive domain events (`transfer.completed`, `claim.adjudicated`, `vend.token.generated`, etc.).

### XVII.5 Business Monetisation Model

NextHub generates revenue for the platform operator through four mechanisms:

**Transaction Fees.** Each domain transaction incurs a configurable platform fee posted to TigerBeetle via `nexthubBillingRouter`. Default fee schedules:

| Domain | Fee Model | Default Rate |
|---|---|---|
| Remittance | % of transfer amount | 0.5% (min ₦500) |
| Healthcare | Fixed per claim | ₦200 per claim submitted |
| Insurance | % of premium collected | 0.3% |
| SCF | % of invoice face value | 0.2% (dynamic discounting) |
| G2P | Fixed per beneficiary | ₦50 per disbursement |
| Energy | Fixed per vend | ₦30 per token |
| CBDC | % of transfer | 0.1% |

**API Access Tiers.** Third parties pay a monthly subscription for API access:

| Tier | RPM | Domains | Monthly Fee |
|---|---|---|---|
| Starter | 60 | 1 | ₦50,000 |
| Growth | 300 | 3 | ₦200,000 |
| Enterprise | 1,200 | All 7 | ₦750,000 |
| Government | 3,000 | All 7 + G2P priority | Custom |

**White-Label Licensing.** Banks and fintechs can deploy NextHub as a private hub under their own branding. The platform operator charges a one-time setup fee plus annual maintenance.

**Data & Analytics.** Aggregated (anonymised) transaction data across domains is packaged as a data product for financial intelligence, credit scoring, and market research.

---

## Part XVIII — Wave 218 Platform Enhancements

The following 20 enhancements were implemented in Wave 218 and are available via `trpc.wave218.*`:

| # | Enhancement | Router | Description |
|---|---|---|---|
| 1 | Domain Health Heartbeat | `wave218.domainHealth.getAll` | Real-time health check for all 7 domains — error rate, TPS, latency |
| 2 | Cross-Domain Unified Search | `wave218.crossDomainSearch.search` | Search transactions across all 7 domain tables simultaneously |
| 3 | Domain SLA Breach Tracker | `wave218.domainSLA.getBreaches` | P95/P99 latency vs SLA threshold per domain |
| 4 | Domain Audit Log | `wave218.domainAudit.list` | Immutable event trail for all domain operations |
| 5 | Domain API Key Management | `wave218.domainApiKeys.create/list/revoke` | Create, list, and revoke per-domain API keys |
| 6 | Domain Throughput Analytics | `wave218.domainAnalytics.getThroughput` | Hourly TPS buckets and 30-day cross-domain volume |
| 7 | Domain Fee Ledger | `wave218.domainFeeLedger.getSummary` | Platform revenue per domain per currency |
| 8 | Retry Queue | `wave218.domainRetry.list/retryNow` | Failed transaction retry management |
| 9 | Cross-Domain Reconciliation | `wave218.crossDomainRecon.generateReport` | 7-domain settlement summary for any date range |
| 10 | Compliance Flag Propagation | `wave218.complianceFlags.flagEntity` | AML/sanctions flag propagated across all domain tables |
| 11 | APISIX Route Health | `wave218.apisixHealth.getRouteHealth` | Upstream latency and error rates for all APISIX routes |
| 12 | Bulk Status Update | `wave218.bulkStatusUpdate.update` | Admin override of transaction status across any domain |
| 13 | Domain Notification Prefs | `wave218.domainNotifPrefs.get/upsert` | Per-merchant per-domain alert preferences |
| 14 | Domain Cost Centre Tagging | `wave218.costCentre.list/upsert` | OpEx allocation and budget tracking per domain |
| 15 | Status Dot Indicators | Layout.tsx `renderNavItem` | Green/amber/red dots on sidebar nav items |
| 16 | Hover Tooltips | Layout.tsx `Tooltip` | Descriptive tooltip on hover for all Domain Expansion items |
| 17 | Wave-Coloured Badges | Layout.tsx badge renderer | Teal badges for W2xx items, indigo for NextHub items |
| 18 | DomainTableToolbar | `DomainTableToolbar.tsx` | Shared filter/sort/export toolbar for all domain pages |
| 19 | SortableTableHeader | `SortableTableHeader.tsx` | Clickable column headers with sort direction indicator |
| 20 | Domain Overview Dashboard | `/domains/overview` | Unified command-centre with live metrics for all 7 domains |

---

## Part XIX — Wave 219 Roadmap

| Wave | Domain | Key Deliverables |
|---|---|---|
| 219 | Core | Kafka topics for all 7 domains; APISIX route configs; participant limits |
| 220 | CBDC | AtomicSwapWorkflow (CBDC ↔ commercial bank money); eNaira webhook integration |
| 221 | Healthcare | NHIA real-time eligibility API; hospital network registry |
| 222 | Remittance | Correspondent banking network; SWIFT gpi integration |
| 223 | SCF | Secondary market invoice trading; tokenised invoice NFT |
| 224 | G2P | Biometric beneficiary verification; offline disbursement via USSD |
| 225 | Energy | Carbon credit settlement per kWh; renewable energy certificate |

---

[51]: https://apisix.apache.org/docs/apisix/plugins/key-auth/ "Apache APISIX key-auth Plugin"
[52]: https://apisix.apache.org/docs/apisix/plugins/limit-count/ "Apache APISIX limit-count Plugin"
[53]: https://temporal.io/docs/concepts/what-is-a-workflow "Temporal Workflow Concepts"
[54]: https://tigerbeetle.com/blog/2023-07-11-we-put-a-distributed-database-in-the-browser "TigerBeetle Architecture"
[55]: https://www.fatf-gafi.org/en/topics/virtual-assets.html "FATF Travel Rule for Virtual Assets"
