...
import { TrendingUp, Edit2, Save, X, RefreshCw, AlertTriangle, Shield } from "lucide-react";
...
export default function NDCPositionLimitEditor() {
  ...
  const startEdit = (row: LimitRow) => {
    setEditingId(row.id);
    setEditValues({ positionLimit: String(row.positionLimit), ndcCap: String(row.ndcCap) });
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Shield className="h-6 w-6 text-blue-500" /> NDC & Position Limit Editor</h1>
          <p className="text-muted-foreground text-sm mt-1">Manage Net Debit Cap and position limits for all DFSP participants</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2"><TrendingUp className="h-4 w-4" /> Participant Limits</CardTitle>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && !limits?.length && <TableRow><TableCell colSpan={10} className="text-center py-8 text-muted-foreground">No participants found.</TableCell></TableRow>}
              {limits?.map((row) => {
                ...
                return (
                  <TableRow key={row.id}>
                    ...
                    <TableCell>
                      {isEditing ? (
                        <Input
                          ...
                          type="number"
                          value={editValues.positionLimit}
                    ...
                    <TableCell>
                      <span className={`font-medium text-sm ${utilizationColor(posPct)}`}>{posPct}%</span>
                    </TableCell>
                    <TableCell>
                      {isEditing ? (
                        <Input
                    ...
                    <TableCell>
                      <span className={`font-medium text-sm ${utilizationColor(ndcPct)}`}>{ndcPct}%</span>
                    </TableCell>
                    <TableCell>
