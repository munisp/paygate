...
export const consumerQrPayRouter = router({
  ...
  resolve: protectedProcedure
    ...
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "DB unavailable" });
      // QR codes are generated by the merchant qrPayments.generate procedure.
      // They encode qrId which we resolve to merchant info.
      if (!input.qrId.startsWith("qr_")) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Invalid QR code format" });
      }
      // In production, QR codes would be stored in a DB table.
      // For now, return a valid placeholder — the frontend passes amount at pay time.
      return {
        valid: true,
        qrId: input.qrId,
        merchantName: "PayGate Merchant",
        currency: "NGN",
      };
    }),
